function generateCurrent(cid)
    ...
    local getCurrentAccount = exports.ghmattimysql:executeSync('SELECT * FROM bank_cards WHERE citizenid = ?', { self.cid })
    ...
    local bankStatement = exports.ghmattimysql:executeSync('SELECT * FROM bank_statements WHERE account = ? AND citizenid = ? ORDER BY record_id DESC LIMIT 30', { 'Current', self.cid })
    ...
    self.updateItemPin = function(pin)
        ...
        if item[1] ~= nil then
            ...
            exports.ghmattimysql:execute("UPDATE `stored_items` SET `metaprivate` = ? WHERE `record_id` = ?", { recode, item[1].record_id }, function(done)
                if done == 1 then
                    success = true
                else
                    success = false
    ...
    self.saveAccount = function()
        local success 
        local processed = false
        exports.ghmattimysql:execute("UPDATE `bank_accounts` SET `amount` = ? WHERE `character_id` = ? AND `record_id` = ?", { self.balance, self.cid, self.aid }, function(success1)
            if success1 > 0 then
                success = true
            else
                success = false
    ...
    rTable.ToggleDebitCard = function(toggle)
        exports.ghmattimysql:execute("UPDATE `bank_accounts` SET `cardLocked` = ? WHERE `character_id` = ? AND `record_id` = ?", { toggle, self.cid, self.aid }, function(rowsChanged)
    ...
    rTable.generateNewCard = function(pin, scc)
        ...
        if not self.cardActive then
            ...
            exports.ghmattimysql:execute('UPDATE bank_cards SET cardnumber = ?, cardPin = ?, cardDecrypted = ?, cardActive = ?, cardLocked = ?, cardType = ? WHERE citizenid = ? AND record_id = ?', {
                cardNumber,
                pinSet,
                false,
                1,
                ...
                if self.source ~= -1 then
                    ...
                    if selectedCard == "visa" then
                        xPlayer.addInventoryItem('visa', 1)
                    elseif selectedCard == "mastercard" then
                        xPlayer.addInventoryItem('mastercard', 1)
                    end
                end
            end)
        end
    ...
    rTable.UpdateDebitCardPin = function(pin)
        exports.ghmattimysql:execute("UPDATE `bank_accounts` SET `cardPin` = ? WHERE `character_id` = ? AND `record_id` = ?", { pin, self.cid, self.aid }, function(rowsChanged)
            if rowsChanged == 1 then
                self.cardPin = pin
    ...
    rTable.AddMoney = function(amt, text)
        ...
        if type(amt) == "number" and text then
            ...
            if successBank then
                ...
                exports.ghmattimysql:execute("INSERT INTO `bank_statements` (`account`, `character_id`, `account_number`, `sort_code`, `deposited`, `withdraw`, `balance`, `date`, `type`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", {
                    "Current",
                    self.cid,
                    self.account,
                    self.sortcode,
...
function generateSavings(cid)
    ...
    self.cid = cid
    self.source = -1
    local getSavingsAccount = exports.ghmattimysql:executeSync('SELECT * FROM bank_accounts WHERE citizenid = ? AND account_type = ?', { self.cid, 'Savings' })
    if getSavingsAccount[1] ~= nil then
        self.aid = getSavingsAccount[1].record_id
        self.balance = getSavingsAccount[1].amount
    end
    local stats = exports.ghmattimysql:executeSync('SELECT * FROM bank_statements WHERE account = ? AND citizenid = ? ORDER BY record_id DESC LIMIT 30', { 'Savings', self.cid })
    self.bankStatement = stats

    self.saveAccount = function()
        exports.ghmattimysql:execute('UPDATE bank_accounts SET amount = ? WHERE citizenid = ? AND record_id = ?', { self.balance, self.cid, self.aid }, function(success)
            if success then
                return true
            else
                return false
    ...
    rTable.AddMoney = function(amt, text)
        if type(amt) == "number" and text then
            ...
            local time = os.date("%Y-%m-%d %H:%M:%S")
            exports.ghmattimysql:execute('INSERT INTO bank_statements (citizenid, account, deposited, withdraw, balance, date, type) VALUES (?, ?, ?, ?, ?, ?, ?)', {
                self.cid,
                'Saving',
                amt,
                0,
...
function createSavingsAccount(cid)
    ...
    local success = false
    exports.ghmattimysql:execute('INSERT INTO bank_accounts (citizenid, amount, account_type) VALUES (?, ?, ?)', { cid, 0, 'Savings' }, function(result)
        savingsAccounts[cid] = generateSavings(cid)
        success = true
        completed = true
    end)
