...
function generatebusinessAccount(acc, sc, bid)
    ...
    local bankAccount = exports.ghmattimysql:executeSync('SELECT * FROM bank_accounts WHERE account_number = ? AND sort_code = ? AND businessid = ?', { self.accountNumber, self.sortCode, self.bid })
    ...
    self.saveAccount = function()
        exports.ghmattimysql:execute("UPDATE `bank_accounts` SET `amount` = ? WHERE `record_id` = ?", { self.balance, self.account_id })
    end
    ...
    rTable.getBankStatement = function(limit)
        ...
        local res = exports.ghmattimysql:executeSync("SELECT * FROM `bank_statements` WHERE `account` = 'business' AND `business` = ? AND `account_number` = ? AND `sort_code` = ? AND `businessid` = ? LIMIT ?", {
    ...
    rTable.deductBalance = function(amt, text)
        if type(amt) == "number" and text then
            if amt <= self.balance then
                ...
                exports.ghmattimysql:execute("INSERT INTO `bank_statements` (`account`, `business`, `businessid`, `account_number`, `sort_code`, `withdraw`, `balance`, `type`) VALUES(?, ?, ?, ?, ?, ?, ?, ?)", {
                    'business',
                    self.account_for,
                    self.bid,
                    self.accountNumber,
    ...
    rTable.addBalance = function(amt, text)
        if type(amt) == "number" and text then
            self.balance = self.balance + amt
            exports.ghmattimysql:execute("INSERT INTO `bank_statements` (`account`, `business`, `businessid`, `account_number`, `sort_code`, `deposited`, `balance`, `type`) VALUES(?, ?, ?, ?, ?, ?, ?, ?)", {
                'business',
                self.account_for,
                self.bid,
                self.accountNumber,
...
function createbusinessAccount(accttype, bid, startingBalance)
    ...
    local checkExists = exports.ghmattimysql:executeSync("SELECT * FROM `bank_accounts` WHERE `business` = ? AND `businessid` = ?", { accttype, bid })
    if checkExists[1] == nil then
        local sc = math.random(100000,999999)
        local acct = math.random(10000000,99999999)
        exports.exports.ghmattimysql:execute("INSERT INTO `bank_accounts` (`business`, `businessid`, `account_number`, `sort_code`, `amount`, `account_type`) VALUES (?, ?, ?, ?, ?, ?)", {
            accttype,
            bid,
            acct,
            sc,
